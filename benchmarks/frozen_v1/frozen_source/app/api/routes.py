...
async def forecast(
    ...
    df, report = await _read_upload(file)
    try:
        result = await run_in_threadpool(create_forecast, df, settings)
    except MarketForgeError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def backtest(
    ...
    df, report = await _read_upload(file)
    try:
        result = await run_in_threadpool(run_walk_forward_backtest, df, settings)
...
async def compare(
    ...
    df, report = await _read_upload(file)
    try:
        result = await run_in_threadpool(compare_baselines, df, settings)
    except (ValueError, MarketForgeError) as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    result["data_report"] = report.as_dict()
    result["data_profile"] = dataframe_profile(df)
