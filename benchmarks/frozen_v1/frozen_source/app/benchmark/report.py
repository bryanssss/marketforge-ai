...
def _pair_evidence(
    ...
    origin_step = int(spec.raw["evaluation"]["origin_step"])
    ...
    for (symbol, horizon), task in matched.groupby(["symbol", "horizon"], sort=True):
        ...
        ci = moving_block_bootstrap_mean(
            ...
            seed=int(stats["seed"]) + int(horizon),
        )
...
def build_report(
    ...
    *,
    ...
    if complete_tasks != expected_tasks:
        reasons.append(f"Only {complete_tasks} of {expected_tasks} tasks have matched predictions.")
    if any(
        task["matched_n"] < int(gate["minimum_observations_per_task"])
        for task in task_reports
    ):
        reasons.append("At least one task has too few unique matched forecast origins.")
    if not math.isfinite(global_ci.upper) or global_ci.upper >= 0:
        reasons.append(
            "The hierarchical 95% confidence interval does not show a global relative loss reduction against Kronos."
        )
    if win_rate < float(gate["minimum_task_win_rate"]):
        reasons.append("Task win rate is below the pre-registered threshold.")
    if significant_win_rate < float(gate["minimum_significant_task_win_rate"]):
        reasons.append("Holm-adjusted significant task win rate is below the threshold.")
    if (
        not math.isfinite(mean_relative_improvement)
        or mean_relative_improvement
        < float(gate["minimum_mean_relative_mae_improvement"])
    ):
        reasons.append("Mean relative MAE improvement is below the pre-registered minimum.")
    if (
        not math.isfinite(mean_coverage_error)
        or mean_coverage_error
        > float(gate["maximum_mean_absolute_coverage_error_80"])
    ):
        reasons.append("Candidate central-80% interval calibration error is too large.")
    if gate.get("must_beat_sanity_comparator") and (
        not math.isfinite(sanity_ci.estimate) or sanity_ci.estimate >= 0
    ):
        reasons.append("The candidate did not beat the pre-registered naive sanity comparator.")
