...
def download(
    ...
    request = urllib.request.Request(
        url, headers={"User-Agent": "MarketForge-Benchmark/0.3"}
    )
...
def freeze_data(
    ...
    *,
    ...
    for symbol in spec.assets:
        ...
        frame = pd.concat(parts, ignore_index=True).sort_values("timestamp").drop_duplicates("timestamp", keep="last")
        start = pd.Timestamp(spec.raw["data"]["context_start"])
        end = pd.Timestamp(spec.raw["data"]["holdout_end_exclusive"])
...
def verify_data_lock(
    ...
    if lock.get("status") != "verified":
        problems.append("Data lock status is not verified.")
    if lock.get("provider_checksum_verified") is not True:
        problems.append("Provider archive checksums are not marked verified.")
    if spec is not None and lock.get("benchmark_id") != spec.benchmark_id:
        problems.append("Data lock benchmark identifier does not match the specification.")
    for item in lock.get("datasets", []):
        path = root / item["path"]
        if not path.exists():
            problems.append(f"Missing canonical dataset: {path}")
            continue
        if sha256_file(path) != item.get("sha256"):
            problems.append(f"Canonical dataset hash mismatch: {path}")
            continue
        if spec is not None:
            try:
                frame = pd.read_csv(path, parse_dates=["timestamp"])
                validate_canonical_frame(frame, spec, str(item.get("symbol", path.stem)))
            except (OSError, ValueError, pd.errors.ParserError) as exc:
                problems.append(f"Canonical dataset validation failed for {path}: {exc}")
    if spec is not None and len(lock.get("datasets", [])) != len(spec.assets):
        problems.append("Data lock does not contain exactly one dataset per frozen symbol.")
    return problems
